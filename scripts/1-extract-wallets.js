...
// Target blockchain assets we want to retrieve
const TARGET_ASSETS = [
    {
        id: 'BTC_TEST',
    ...
    {
        id: 'ETH_TEST5', // Correct Sepolia testnet ID
...
class FireblocksWalletRetriever {
    ...
    async initializeSDK() {
        try {
            ...
            // Resolve the correct basePath. If the env var is missing or doesn't include "/v1",
            // fall back to the official constant so we avoid 404 errors.
            let resolvedBasePath = process.env.FIREBLOCKS_BASE_URL;
            if (!resolvedBasePath || !/\/v\d+$/.test(resolvedBasePath)) {
                // Either not provided or missing the version suffix – default to SDK constant.
    ...
    async retrieveVaultWallets() {
        try {
            console.log(`\n${colors.cyan}${colors.bright}🏦 STEP 3: Retrieving Vault Wallet Addresses${colors.reset}`);
            console.log('═'.repeat(50));
            ...
            if (!vaultAccount.assets || vaultAccount.assets.length === 0) {
                ...
                console.log(`${colors.blue}💡 This vault might be empty or you might need to create wallets first${colors.reset}`);
                return vaultAccount;
            }

            console.log(`\n${colors.cyan}📋 Assets found in vault:${colors.reset}`);
            vaultAccount.assets.forEach(asset => {
                console.log(`  - ${asset.id}: ${asset.total || '0'} available`);
            });
    ...
    async getWalletAddresses(vaultAccount) {
        try {
            ...
            for (const targetAsset of TARGET_ASSETS) {
                console.log(`\n${colors.yellow}🔍 Looking for ${targetAsset.name} (${targetAsset.id})...${colors.reset}`);
                
                // Find the asset in vault's assets
                const vaultAsset = vaultAccount.assets?.find(asset => asset.id === targetAsset.id);
                
                if (vaultAsset) {
                    console.log(`${colors.green}✅ Found ${targetAsset.name} in vault${colors.reset}`);
                    console.log(`${colors.blue}💰 Balance: ${vaultAsset.total || '0'}${colors.reset}`);
                    
                    // Now get the actual wallet addresses for this asset
                    try {
                        console.log(`📡 Getting wallet addresses for ${targetAsset.id}...`);
                        ...
                        if (addresses && addresses.length > 0) {
                            ...
                            foundAddresses[targetAsset.chain] = {
                                address: primaryAddress.address,
                                legacyAddress: primaryAddress.legacyAddress || null,
                                assetId: targetAsset.id,
                                name: targetAsset.name,
                                balance: vaultAsset.total || '0',
                                addressCount: addresses.length
                            };
                        } else {
                            console.log(`${colors.yellow}⚠️ No addresses found for ${targetAsset.name}${colors.reset}`);
                            console.log(`${colors.blue}💡 Creating wallet and generating first deposit address...${colors.reset}`);
                            ...
                            try {
                                ...
                                const addrResp = await this.fireblocks.vaults.createVaultAccountAssetAddress({
                                    ...
                                    createAddressRequest: {
                                        description: 'Primary address created by script'
                                    }
                                });
                                ...
                                console.error(`${colors.red}❌ Failed to generate address for ${targetAsset.name}:${colors.reset}`, creationError.message);
                                missingAssets.push(targetAsset);
                            }
                        }
                        
                    } catch (addressError) {
                        console.error(`${colors.red}❌ Failed to get addresses for ${targetAsset.name}:${colors.reset}`, addressError.message);
                        missingAssets.push(targetAsset);
                    }
                    
                } else {
                    console.log(`${colors.yellow}⚠️ ${targetAsset.name} not found in vault${colors.reset}`);
                    console.log(`${colors.blue}💡 Creating wallet and generating first deposit address...${colors.reset}`);
            ...
            if (missingAssets.length > 0) {
                console.log(`\n${colors.yellow}📋 Missing Assets (need wallet creation):${colors.reset}`);
                missingAssets.forEach(asset => {
                    console.log(`  - ${asset.name} (${asset.id})`);
                });
                console.log(`\n${colors.blue}💡 To create missing wallets, use:${colors.reset}`);
                console.log(`${colors.blue}   fireblocks.vaults.createVaultAsset({ vaultAccountId, assetId })${colors.reset}`);
            }
    ...
    generateSummary() {
        ...
        if (foundCount > 0) {
            ...
            console.log(`${colors.red}❌ No wallet addresses were found${colors.reset}`);
            console.log(`${colors.yellow}🔧 You may need to create wallets for the target assets first${colors.reset}`);
            console.log(`${colors.yellow}💡 Check the Fireblocks Console or create wallets via API${colors.reset}`);
        }

        return {
            success: errorCount === 0 && foundCount > 0,
            foundCount,
            totalTargets,
            errorCount,
    ...
    async run() {
        console.log(`${colors.bright}${colors.cyan}🔍 FIREBLOCKS WALLET ADDRESS RETRIEVAL${colors.reset}`);
...
const isMainModule = process.argv[1] && process.argv[1].endsWith('1-extract-wallets.js');
